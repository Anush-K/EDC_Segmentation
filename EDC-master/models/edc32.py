# models/edc32.py
#
# BACKBONE ABLATION VARIANT: ResNeXt-50 32x4d encoder + decoder
# -------------------------------------------------------------------------
# Standalone file — does NOT modify models/edc.py.
# Same RQASW (Novelty 1) fusion logic as R50_R50, only the backbone differs.
#
# CHANNEL NOTE:
# ResNeXt50-32x4d uses Bottleneck (expansion=4), so layer4 output is 2048
# channels — identical to ResNet50 and WideResNet50. Safe drop-in with
# inplanes=[2048], using the existing resnext50_32x4d_decoder.

from models.resnet import resnext50_32x4d
from models.resnet_decoder import resnext50_32x4d_decoder
...
class RX50_RX50(nn.Module):
    """
    ResNeXt-50 32x4d encoder + ResNeXt-50 32x4d decoder for EDC anomaly detection.
    ...
    def __init__(
        ...
        super().__init__()
        self.edc_encoder    = resnext50_32x4d(pretrained=True)
        # ResNeXt50-32x4d layer4 output = 2048 channels (Bottleneck, expansion=4)
        self.edc_decoder    = resnext50_32x4d_decoder(pretrained=False, inplanes=[2048])
