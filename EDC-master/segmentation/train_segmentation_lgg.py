# segmentation/train_segmentation_lgg.py
# NOVELTY 2: HGBL — Heatmap-Guided Boundary-Aware Loss
# For LGG: heatmap guides UNet input (4-channel),
...
from dataset_lgg import LGGSegDataset
...
def train_epoch(model, loader, optimizer, use_hgbl=True):
    ...
    for image, heatmap, mask in tqdm(loader, leave=False, desc='  train'):
        image, heatmap, mask = (image.to(DEVICE), heatmap.to(DEVICE),
                                mask.to(DEVICE))
        optimizer.zero_grad()
...
def run_experiment(use_hgbl, tag=''):
    ...
    full = LGGSegDataset(DATASET_PATH)
    n    = len(full)
    nt   = max(1, int(0.8 * n))
    nv   = n - nt
    ...
    # persistent_workers=True: worker pool created once per experiment,
    # not re-forked every epoch — prevents OSError: Cannot allocate memory
    # on the second run_experiment() call (same crash seen on BUSI).
    ...
    ckpt = os.path.join(SAVE_DIR,
           f"best_unet_lgg_{'hgbl' if use_hgbl else 'baseline'}.pth")
...
def run_ablation():
    ...
    r['+ Multi-scale Boundary (lb=5)'] = (d, i)

    LAMBDA_H = 1.; LAMBDA_B = 0.
    d, i = run_experiment(True, '[row3]')
    r['+ HeatmapxPred guidance (lh=1)'] = (d, i)

    LAMBDA_H = 1.; LAMBDA_B = 5.
    d, i = run_experiment(True, '[row4]')
    r['Full HGBL (lb=5, lh=1)'] = (d, i)

    print("\n" + "=" * 62 + "\n  ABLATION TABLE — LGG\n" + "=" * 62)
    print(f"  {'Method':<44} {'Dice':>6}  {'IoU':>6}")
    print(f"  {'-'*44} {'-'*6}  {'-'*6}")
    for m, (d, i) in r.items():
        print(f"  {m:<44} {d:.4f}  {i:.4f}")
    print("=" * 62)
    return r
