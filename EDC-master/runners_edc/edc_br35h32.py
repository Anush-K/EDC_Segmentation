...
# BACKBONE ABLATION: ResNeXt-50 32x4d encoder/decoder, Br35H dataset
...
from models.edc32 import RX50_RX50          # backbone ablation — untouched edc34.py
...
def run_single_seed(gpu, args, seed):
    ...
    # ✅ BACKBONE ABLATION: ResNeXt-50 32x4d encoder/decoder (models/edc32.py)
    model = RX50_RX50(
...
def main_worker(gpu, args):
    ...
    print("\n======== FINAL EVALUATION METRICS — Br35H / ResNeXt50-32x4d (5-seed ensemble) ========\n")
    print(metrics.to_string(index=False, float_format="%.4f"))

    print("\n======== COMPARISON WITH ORIGINAL ResNet50 RUN ========")
    print(f"{'Metric':<15} {'ResNet50 (yours)':>18} {'ResNeXt50 (this run)':>20} {'Delta':>10}")
    print("-"*66)
    ...
    mis_dir = os.path.join(save_path, "misclassified_br35h32")
    ...
    pd.DataFrame(results,
        ...
        os.path.join(save_path, "results_test_edc_br35h32.csv"), index=False)
    pd.DataFrame(misclassified,
        columns=["S.No","Filename","GT","Pred","Score"]).to_csv(
        os.path.join(save_path, "misclassified_test_edc_br35h32.csv"), index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sn', '--save_name', type=str,      default='edc_br35h32')
