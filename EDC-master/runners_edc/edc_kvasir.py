...
from datasets.dataset_kvasir import AD_Dataset
from datasets.data_utils import get_data_loader

from models.edc import R50_R50
from configs.config_kvasir import DATASET_DIR
...
def main_worker(gpu, args):
    ...
    logger.info(
        f"Train -> Normal: {train_counts.get(0,0)} | "
        f"Abnormal: {train_counts.get(1,0)}"
    )
    logger.info(
        f"Eval  -> Normal: {eval_counts.get(0,0)} | "
        f"Abnormal: {eval_counts.get(1,0)}"
    )

    # ✅ FIX: scale iterations to compensate for small batch size
    # batch=8 needs ~4x more iters than batch=32 to see same data
    n_train = len(train_dset)
    recommended_iters = max(
        args.num_train_iter,
        (n_train // args.batch_size) * 5   # 5 full epochs
    )
    if recommended_iters > args.num_train_iter:
        logger.info(
            f"[INFO] Scaling num_train_iter: "
            f"{args.num_train_iter} → {recommended_iters} "
            f"(5 epochs × {n_train} samples ÷ batch {args.batch_size})"
        )
        args.num_train_iter  = recommended_iters
        args.num_eval_iter   = max(args.num_eval_iter,
                                   recommended_iters // 5)
    ...
    print("\n======== FINAL EVALUATION METRICS — KVASIR ========\n")
    print(metrics.to_string(index=False, float_format="%.4f"))

    # ✅ FIX: log score distribution to verify model is separating classes
    ...
    mis_dir         = os.path.join(save_path, "misclassified_kvasir")
    ...
    pd.DataFrame(results, columns=["S.No","Filename","GT","Pred","Score"]).to_csv(
        os.path.join(save_path, "results_test_edc_kvasir.csv"), index=False)
    pd.DataFrame(misclassified, columns=["S.No","Filename","GT","Pred","Score"]).to_csv(
        os.path.join(save_path, "misclassified_test_edc_kvasir.csv"), index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sn', '--save_name', type=str,      default='edc_kvasir')
    ...
    parser.add_argument('-ds','--dataset',    type=str,      default='kvasir')
