...
def main_worker(gpu, args):
    ...
    logger.info(
        f"TrainSet: {len(train_dset)} | "
        f"EvalSet: {len(eval_dset)}"
    )

    train_counts = Counter(
        np.array(train_dset.targets).tolist()
    )

    eval_counts = Counter(
        np.array(eval_dset.targets).tolist()
    )
    ...
    logger.info(
        f"Eval -> Normal: "
        f"{eval_counts.get(0,0)} | "
        f"Abnormal: {eval_counts.get(1,0)}"
    )

    generator_lb = torch.Generator().manual_seed(
        args.seed
    )
    ...
    logger.info(
        ...
        f"{count_parameters(runner.model):,}"
    )
    ...
    print(
        f"Best Threshold (F1-optimised): {best_thr:.4f}"
    )
    ...
    mis_dir = os.path.join(
        save_path,
        "misclassified_covid19"
    )

    norm_as_abn_dir = os.path.join(
        mis_dir,
        "Normal_as_Abnormal"
    )

    abn_as_norm_dir = os.path.join(
        mis_dir,
        "Abnormal_as_Normal"
    )
    ...
    for i, (score, gt, img_path) in enumerate(
        ...
        results.append(
            [i, fname, int(gt), pred, float(score)]
        )
        ...
        if pred != int(gt):
            ...
            if os.path.exists(img_path):
                ...
                if gt == 0 and pred == 1:
                    ...
                    shutil.copy(
                        img_path,
                        os.path.join(norm_as_abn_dir, fname)
                    )
                    ...
                    shutil.copy(
                        img_path,
                        os.path.join(abn_as_norm_dir, fname)
                    )

    pd.DataFrame(
        results,
        columns=["S.No", "Filename", "GT", "Pred", "Score"]
    ).to_csv(
        os.path.join(
            save_path,
            "results_test_edc_covid19.csv"
        ),
        index=False
    )
    ...
    pd.DataFrame(
        ...
        os.path.join(
            save_path,
            "misclassified_test_edc_covid19.csv"
        ),
        index=False
    )
    ...
    print(
        f"  Normal->Abnormal : {mis_normal}/{total_normal} "
        f"(acc {1 - mis_normal / max(total_normal, 1):.4f})"
    )

    print(
        f"  Abnormal->Normal : {mis_abnormal}/{total_abnormal} "
...
if __name__ == "__main__":
    ...
    if (
        ...
        raise Exception(
            f"Model exists at {save_path}."
        )

    if (
        args.resume
        and args.load_path is None
    ):
